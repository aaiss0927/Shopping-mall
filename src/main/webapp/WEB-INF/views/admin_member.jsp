...
<nav>
    <ul>
        <li style="margin-left: 25px;"><a href="/admin"><strong style="font-size:25px; color:cornflowerblue">DUNE</strong><span style="font-size:12px; color:red">Admin</span></a></li>
        <li style="margin-left: 90px;"><a href="/admin/member/paging">회원 관리</a></li>
        <li><a href="/admin/product/paging">상품 관리</a></li>
        <li><a href="/admin/board/paging">게시판 관리</a></li>
    </ul>
</nav><br><br>
<section>
    <div style="width:60%; margin:auto;">
        <strong style="font-size:25px">회원 관리</strong><hr style="background:skyblue; height:2.5px; border:0;"><br>
        <table>
            <thead>
            <tr>
                <th>번호</th><th>ID</th><th>이름</th><th>이메일</th><th>전화번호</th><th>삭제</th>
            </tr>
            </thead>
            <tbody>
            <c:forEach items="${memberList}" var="member">
                <tr>
                    <td>${member.manage_id}</td>
                    <td>${member.memberId}</td>
                    <td>${member.memberName}</td>
                    <td>${member.memberEmail}</td>
                    <td>${member.memberTel}</td>
                    <td><a href="/admin/member/delete?manage_id=${member.manage_id}" style="text-decoration: none">삭제</a></td>
                </tr>
            </c:forEach>
            </tbody>
        </table><br>
        ...
        <div align="center">
            <c:choose>
                ...
                <c:otherwise>
                    <a href="/admin/member/paging?page=${paging.page-1}" style="text-decoration: none">[이전]</a>
                </c:otherwise>
            </c:choose>
            ...
            <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i" step="1">
                <c:choose>
                    <c:when test="${i eq paging.page}">
                        <input type="button" value="${i}" style="color:red">
                    </c:when>

                    <c:otherwise>
                        <input type="button" value="${i}" onclick="location.href='/admin/member/paging?page=${i}'">
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <a href="/admin/member/paging?page=${paging.page+1}" style="text-decoration: none">[다음]</a>
                </c:otherwise>
            </c:choose>
        </div>
    </div>
